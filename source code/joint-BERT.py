...
def build_model(max_seq_length): 
    ...
    dense1 = tf.keras.layers.Dense(128, activation='relu')(bert_output1)

    concat = tf.keras.layers.concatenate([dense, dense1], axis=-1)

    pred = tf.keras.layers.Dense(1, activation='sigmoid')(concat)
    
    model = tf.keras.models.Model(inputs=[bert_inputs,bert_inputs1], outputs=pred)
...
model.fit(
    [train_input_ids, train_input_masks, train_segment_ids,train_input_ids1, train_input_masks1, train_segment_ids1], 
    train_labels,
...
y_prob = model.predict([test_input_ids, test_input_masks,  test_segment_ids, test_input_ids1, test_input_masks1,  test_segment_ids1])
#print(y_prob)
