...
dataTrain, dataLabel = parse_training("english-balanced.tsv")
...
def build_model(max_seq_length, vocab): 
    ...
    #in_text = tf.keras.layers.Input(shape=(max_seq_length,), name="input_texts")
    bert_inputs = [in_id, in_mask, in_segment]
    
    bert_output = BertLayer(n_fine_tune_layers=3, pooling="first")(bert_inputs)
    #embedding_layer = tf.keras.layers.Embedding(vocab+1, 300, input_length=100)(in_text)
    
    #lstm_layer = tf.keras.layers.LSTM(64)(embedding_layer)
    dense = tf.keras.layers.Dense(128, activation='relu')(bert_output)
    #dense2 = tf.keras.layers.Dense(64, activation='relu')(lstm_layer)
    
    #concat = tf.keras.layers.concatenate([dense, dense2], axis=-1)
...
#for test_dataset in ['french', 'german', 'indonesian', 'italian', 'portuguese', 'spanish']:
for test_dataset in ['german_english_ross','german_english_hasoc','indonesian_english_ibrahim','indonesian_english_alfina','spanish_english_basile','spanish_english_pereira']:
    test_path = '{0}.tsv'.format(test_dataset)
